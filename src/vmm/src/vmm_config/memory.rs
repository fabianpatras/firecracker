...
impl MemoryBuilder {
    ...
    pub fn memory_from_config(cfg: MemoryDeviceConfig) -> Result<MutexMemory> {
    ...
    pub fn set(&mut self, cfg: MemoryDeviceConfig) -> Result<()> {
        self.insert_device(Self::memory_from_config(cfg)?)?;

        Ok(())
    }

    /// Inserts an existing memory device.
    pub fn insert_device(&mut self, memory: MutexMemory) -> Result<()> {
